...
- `adapters/workspace_chat_adapter.py`: custom SSE workspace chat adapter
...
- Base install supports `--adapter workspace-chat`.
...
| `workspace-chat` | `WORKSPACE_CHAT_URL` | `WORKSPACE_CHAT_API_KEY`, `WORKSPACE_CHAT_TIMEOUT_SECONDS` (default `30`) |
...
Useful flags:
- `--adapter fake|openai|openai-compat|ollama|workspace-chat`
...
- Use `workspace-chat` for custom SSE endpoints that accept `{"message": "<prompt>"}` and stream `data:` events with `textResponse` chunks.
